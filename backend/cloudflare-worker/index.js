...
A) КОРРЕКТНОСТЬ ТИПА ИСТОЧНИКА. Например, если выбран тип «пресс-служба», а конкретный источник записан как «уборщица», это несоответствие категории.
...
Пример: уборщица может быть подходящим источником того, что она лично увидела в коридоре. Она также МОЖЕТ знать решение администрации, если присутствовала при разговоре или получила официальную информацию как сотрудник. Но это основание знания должно быть указано; её профессия сама по себе ничего не доказывает и ничего не опровергает.

КАК ДАВАТЬ ОБРАТНУЮ СВЯЗЬ
— Выбери максимум 6 главных проблем. Не перечисляй всё подряд.
— Сначала critical, потом important, потом note.
— Объединяй повторяющиеся замечания. Не делай отдельные карточки про одно и то же противоречие.
— Для каждого замечания укажи конкретный ответ/поле ученика, которое вызвало проблему.
— Формат мышления для каждого замечания: ПРОБЛЕМА → ПОЧЕМУ → ЧТО ПРОВЕРИТЬ → ЧТО СДЕЛАТЬ ДАЛЬШЕ.
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET") {
      return jsonResponse({ ok: true, service: "MMT DVI AI Review", model: MODEL, calibration: "mmt-2026-08-18-a" }, 200, origin);
    }
    ...
    try {
      ...
      return jsonResponse({
        ...
        meta: { provider: "cloudflare-workers-ai", modelVersion: "glm-4.7-flash", calibration: "mmt-2026-08-18-a" }
      }, 200, origin);
    } catch (error) {
