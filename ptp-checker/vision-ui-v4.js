// V-CHECK visual AI for extracted frames. Cache-safe v4 loader.
(() => {
  ...
  async function analyze(el){
    ...
    const r=await fetch(API_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'videoVision',imageBase64:await base64(blob),mimeType:blob.type||'image/jpeg'}),cache:'no-store'});
