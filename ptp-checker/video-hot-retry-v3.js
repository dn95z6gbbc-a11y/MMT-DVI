// V-CHECK Hot retry v3: reject partial repaired reports and retry only final AI analysis.
(() => {
  const API_URL = 'https://functions.yandexcloud.net/d4ejdq5v5too7egeop63';
  if (window.__VCHECK_HOT_RETRY_V3__) return;
  window.__VCHECK_HOT_RETRY_V3__ = '3.0';
  ...
  function hasCompleteHotSchema(data) {
    ...
    // Hot has many mandatory + deep checks. A 1–3 item object is a repaired/truncated JSON,
    // not a real report. Eight is deliberately conservative and still below the expected total.
  ...
  window.fetch = async function hotRetryV3Fetch(input, init = {}) {
    ...
    for (let attempt = 2; attempt <= MAX_ATTEMPTS; attempt++) {
      console.warn(
        `V-CHECK Hot: final report incomplete/malformed; retrying final analysis (${attempt}/${MAX_ATTEMPTS}).`,
        state.message,
      ...
      setProgress(
        state.incompleteSchema
          ? `ИИ вернул неполный отчёт. Повторяем только итоговый анализ (${attempt}/${MAX_ATTEMPTS})…`
          : `Финальный ИИ-ответ пришёл некорректно. Повторяем только итоговый анализ (${attempt}/${MAX_ATTEMPTS})…`
      );
    ...
    console.warn('V-CHECK Hot: all final-analysis attempts returned incomplete/malformed reports.');
    setProgress('Видео обработано, но ИИ несколько раз вернул неполный итоговый отчёт.', 'bad');
    return incompleteResponse();
  };

  console.log('V-CHECK Hot retry v3.0 loaded');
})();
