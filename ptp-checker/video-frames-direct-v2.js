(function(){
  ...
  function loadVisionUi(){
    if(d.getElementById('vcheckVisionUiScriptV4'))return;
    const s=d.createElement('script');
    s.id='vcheckVisionUiScriptV4';
    s.src='./vision-ui-v4.js';
    s.defer=true;
  ...
  async function captureOne(sourceUrl,target,width,height){
    const video=d.createElement('video');
    video.preload='auto';
    video.muted=true;
    video.playsInline=true;
  ...
  async function buildFrames(file){
    const myRun=++runId;
    revokePreviewUrls();
    ...
    try{
      const meta=await readMeta(sourceUrl);
      ...
      for(let i=0;i<fractions.length;i++){
        ...
        try{
          ...
          const shot=await captureOne(sourceUrl,target,width,height);
  ...
  function bind(){
    ensureStyles();
    ensureBox();
    if(d.documentElement.dataset.vcheckFramesDirect==='2')return;
    d.documentElement.dataset.vcheckFramesDirect='2';
