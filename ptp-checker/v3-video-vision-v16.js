(function(){
  ...
  async function callVision(payload){
    ...
    if(!data.ok)throw new Error(data.message||data.error||`Ошибка ${response.status}`);
    return data;
  }
  ...
  function ensureControls(d){
    ...
    controls.innerHTML='<button type="button" id="vcheckVisionButton" disabled>Проверить кадры ИИ</button><span id="vcheckVisionStatus">Готовим кадры…</span>';
  ...
  function refreshReadyState(d){
    ...
    const target=Number(state.target)||12;

    if(state.running){
      button.disabled=true;
      button.textContent='Проверить кадры ИИ';
      status.textContent=`Готовим кадры: ${frames.length}/${target}…`;
      return;
    }
    ...
    if(frames.length>=MIN_FRAMES){
      ...
      status.textContent=`${frames.length} кадров готовы. Можно запустить тест зрения.`;
    }else{
      button.disabled=true;
      button.textContent='Проверить кадры ИИ';
      status.textContent=frames.length?`Получено ${frames.length} кадров. Нужно минимум ${MIN_FRAMES}.`:'Сначала дождитесь подготовки кадров.';
    }
  }
  ...
  async function runVision(d){
    ...
    const frames=Array.isArray(window.__VCHECK_VIDEO_FRAMES__)?window.__VCHECK_VIDEO_FRAMES__.slice():[];
    ...
    async function worker(){
      while(true){
        ...
        const f=frames[i];
        const items=d.querySelectorAll('#vcheckVideoFrames .vf-item:not(.vf-failed)');
        const item=items[i];
        let badge=null;
        if(item){
        ...
        try{
          ...
          results[i]={index:f.index,timeSeconds:f.timeSeconds,analysis:result.analysis||null,model:result.model||null,imageBytes:result.imageBytes||f.size||null};
          if(badge){badge.className='vf-ai ok';badge.textContent=frameLabel(result);}
        }catch(error){
          results[i]={index:f.index,timeSeconds:f.timeSeconds,error:error?.message||'Ошибка анализа'};
          if(badge){badge.className='vf-ai err';badge.textContent='ошибка';}
        }finally{
          done++;
  ...
  function attach(){
    ...
    const box=d.getElementById('vcheckVideoFrames');
    if(box&&!box.dataset.visionObserver){
      box.dataset.visionObserver='1';
